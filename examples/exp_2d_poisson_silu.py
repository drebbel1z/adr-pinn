"""
Experiment: 2D Poisson Adaptation -- SiLU Network (Figure 4 / Table 6)

Trains a 3-hidden-layer SiLU PINN on the 2D Poisson equation
...
Output: examples/pinn_poisson_silu.pdf

Run:
    python examples/exp_2d_poisson_silu.py
"""
...
def run():
    ...
    for label, layers in ADAPT_CONFIGS:
        ...
        # d_eff
        d = get_d_eff(model, pde_fn, layers, x_pde, x_bc, mode="all", rcond=1e-8)
        pde_cap = d["pde"]
        bc_cap = d["bc"]
    ...
    for row in table_data:
        print(
            f"  {row['label']:<{w}} {row['d_pde']:>10.3f} {row['d_bc']:>10.3f} "
            f"{row['ratio']:>8.3f} {row['rmse']:>14.4e} {row['pde_loss']:>14.4e} {row['time']:>9.2f}s"
        )
    print("=" * 107 + "\n")
    ...
    ax_e.contourf(x_np, y_np, err_m1, levels=levels_err, cmap=cmap_err, extend="max")
    ax_f.contourf(x_np, y_np, err_m2, levels=levels_err, cmap=cmap_err, extend="max")
    c_g = ax_g.contourf(
    ...
    out_path = os.path.join(os.path.dirname(__file__), "pinn_poisson_silu.pdf")
